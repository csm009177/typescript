...
// tagName은 JSX(javascript xml의 줄임말)에서 사용할 수 있는 모든 HTML 태그의 이름입니다.
// props는 위에서 정의한 Props 타입을 따르기 때문에 협업에서도 상당히 편리합니다.
// url은 문자열로, 데이터를 가져올 웹 주소입니다
type FetchExampleProps = {
  tagNamea: keyof React.ReactHTML;
  // keyof는 타입스크립트의 키워드로, 객체의 모든 키를 문자열로 지정합니다
  props: Props;
  url: string;
};
/**
 * 타입스크립트에서 <> 꺽쇠 (태그) 표기법은 제네릭 문법이라고 부르며
 * 프로그래밍 일반에서 상당히 자주 사용되는 패턴을 그대로 벤치 마킹한 것입니다
 * 선언할 때는 마치 매개변수처럼 사용하며, 할당할 때는 타입을 결정합니다
 * "사용할때 타입을 결정한다" 라는 키워드가 핵심이 됩니다
 */

// FetchExample 컴포넌트 정의 : React.FC는 React 함수 컴포넌트를 의미합니다.
const FetchExample: React.FC<FetchExampleProps> = ({ tagName, props, url }) => {
  // useState 훅을 사용하여 컴포넌트의 상태를 관리합니다
  // data 상태는가져온 데이터를 저장하고, error 상태는 에러 메시지를 저장합니다.
  // 너무 많은 상태를 관리하면 코드가 복잡해지기 때문에, 상태는 최소한으로 관리하는 것이 좋으며,
  // useReducer 훅을 사용하면 상태를 더욱 효율적으로 관리할 수 있습니다
  const [data, setData] = useState<String>("로딩중");
  const [error, setError] = useState<string>("");

  // useEffect 혹은 컴포넌트가 마운트될 때 실행됩니다.
  useEffect(() => {
    // fectchData는 비동기 함수로, API에서 데이터를 가져옵니다
    // 바닐라 예제와 다르게 try부분에서 async 키워드를 선언한 것이 차이점 입니다.
    // 리엑트의 작성방식을 따르면서도, 원하는 기능을 구현하기 위한 조치입니다
  ...
  // 컴포넌트의 리턴 부분 : 만약 에러가 있으면 에러 메시지를, 그렇지 않으면 데이터를 표시합니다
